...
test.describe("Home page with no auth", () => {
  ...
  test("check for inputs without labels", async ({ page }) => {
    ...
    const inputsWithoutLabels = await page.evaluate(() => {
      ...
      expect(inputsWithoutLabels.length, `Labels with issues: ${inputsWithoutLabels.toString()}`).toBe(0);
  });
  ...
  test("check for broken images", async ({ page }) => {
    ...
    const brokenImages = await page.evaluate(() => {
      ...
      expect(brokenImages.length, `Labels with issues: ${brokenImages.toString()}`).toBe(0);
  });
});
...
test.describe("Home page customer 01 auth", () => {
  ...
  test("validate product data is visible in UI from API", async ({ page }) => {
    ...
    await test.step('intercept /products', async() => {
      await page.route('https://api.practicesoftwaretesting.com/products**', 
        async(route) => {
...
test('Validate product data is loaded from har file', async({ page }) => {
  ...
  await page.waitForTimeout(3000);
  const productGrid = page.locator('.col-md-9');
  await expect(productGrid).toContainText("Happy Path Plier");
  await expect(productGrid).toContainText("100");
});